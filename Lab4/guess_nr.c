...
static uint16_t get_nr(void)
{
	...
	uint16_t guessed_nr;
	while (1) {
		input_length = input_int("Enter number:", &guessed_nr);   // & skickar adressen !!!!!!!!!!!!!!!!!!!!! //UPPGIFT: ändra koden så att 'input_int()' anropas, returvärdet ska lagras i 'input_length', 'guessed_nr' ska modifieras av 'input_int()'.
