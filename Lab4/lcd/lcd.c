...
void lcd_write_str(char *p_str)
{
	...
	while(*p_str != 0) {            //sålänge pekaren till strängens karaktär inte är tom
		lcd_write(DATA, *p_str);   // skriv ut data
		*p_str++;                  // räknar
		lcd_set_cursor_mode(CURSOR_BLINK);  //så att the cursor blinkar
	}
	*p_str = 0;              //slut
	return;        
